...
def save_grok_token(sso, email="", password=""):
    ...
    data = {"email": _s(email), "sso": sso, "ts": int(time.time())}
    if password:
...
def _build_grok_cookies(sso, email=""):
    """Build browser-export-style cookies JSON from SSO token."""
    now_ts = time.time()
    sso_expiry = now_ts + 180 * 24 * 3600  # ~6 months

    cookies = []
    # sso cookie on .x.ai
    cookies.append({
        "domain": ".x.ai",
        "expirationDate": sso_expiry,
        "hostOnly": False,
        "httpOnly": True,
        "name": "sso",
        "path": "/",
...
def _build_grok_auth_json(credentials, email="", password=""):
    """Build ~/.grok/auth.json format from OAuth credentials."""
    ...
    claims = parse_jwt_payload(access_token) or {}
    user_id = claims.get("sub", "")
    principal_type = claims.get("principal_type", "User")
    principal_id = claims.get("principal_id", user_id)
    team_id = claims.get("team_id", "")
    expires_at = credentials.get("expires_at", "")

    scope_key = f"{XAI_ISSUER}::{XAI_CLIENT_ID}"
...
def save_grok_full_account(sso, email="", password="", proxy=""):
    """Save complete Grok account: SSO + cookies + auth.json (with refresh_token).

    Saves 3 files into tokens/grok/{email}/:
      - sso.json       — SSO token (basic)
      - cookies.json   — browser-export cookie format
      - auth.json      — grok CLI auth.json format (with refresh_token)

    Returns True on success (SSO saved + OAuth conversion succeeded).
    """
    sso = _s(sso)
    if not sso:
        return False
    ...
    # Create per-account subdirectory: tokens/grok/{email}/
    acct_dir = os.path.join(TOKEN_OUTPUT_DIR, "grok", name)
    os.makedirs(acct_dir, exist_ok=True)

    # Also save to legacy flat location for backward compatibility
    pdir = _platform_dir("grok")
    legacy_path = os.path.join(pdir, f"{name}.sso.json")
    sso_data = {"email": _s(email), "sso": sso, "ts": int(time.time())}
    if password:
        sso_data["password"] = password
    with open(legacy_path, "w", encoding="utf-8") as f:
        json.dump(sso_data, f, indent=2, ensure_ascii=False)

    # 1) Save SSO token
    sso_path = os.path.join(acct_dir, "sso.json")
    with open(sso_path, "w", encoding="utf-8") as f:
        json.dump(sso_data, f, indent=2, ensure_ascii=False)
    print(f"  [grok] sso.json saved: {acct_dir}")

    # 2) Save cookies
    cookies = _build_grok_cookies(sso, email)
    cookies_path = os.path.join(acct_dir, "cookies.json")
    with open(cookies_path, "w", encoding="utf-8") as f:
        json.dump(cookies, f, indent=2, ensure_ascii=False)
    print(f"  [grok] cookies.json saved: {acct_dir}")

    # 3) Convert SSO -> OAuth refresh_token -> auth.json
    try:
        ...
        if not proxy:
            try:
                ...
                proxy = os.environ.get("REG_FACTORY_PROXY", "socks5://127.0.0.1:10808")

        print(f"  [grok] converting SSO -> OAuth refresh_token...")
        credentials, cred_email = convert_grok_sso_local(
            sso, proxy=proxy, account_email=email, timeout=90
        )
        rt = credentials.get("refresh_token", "")
        if rt:
            auth_data = _build_grok_auth_json(credentials, email=email, password=password)
            auth_path = os.path.join(acct_dir, "auth.json")
            with open(auth_path, "w", encoding="utf-8") as f:
                json.dump(auth_data, f, indent=2, ensure_ascii=False)
            print(f"  [grok] auth.json saved: {acct_dir}  (refresh_token={rt[:30]}...)")
        else:
            print("  [grok] WARNING: OAuth did not return refresh_token")
    except Exception as e:
        print(f"  [grok] WARNING: OAuth conversion failed: {e}")
        print(f"  [grok] SSO + cookies saved OK. Run 'python tools/sso_to_auth_json.py' later.")

    print(f"  [grok] === All files in: {acct_dir} ===")
    return True
